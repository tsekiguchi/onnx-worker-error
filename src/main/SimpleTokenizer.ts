...
import defaultBPEPath from '../../resources/bpe_simple_vocab_16e6.txt.gz?asset&asarUnpack';
...
export class SimpleTokenizer {
	...
	constructor(config: Partial<SimpleTokenizerConfig> = {}) {
		...
		this.bpeRanks = new Map<string, number>(merges.map((merge, index) => [merge, index]));
		...
		this.cache = new Map<string, string>(specialTokens.map((token) => [token, token]));
	...
	public tokenize(texts: string | string[]): ort.Tensor {
		...
		const allTokens = texts.map((text) => [
			BigInt(this.sotTokenId),
			...this.encode(text).map(BigInt),
			BigInt(this.eotTokenId)
		]);

		const flatResult = new BigInt64Array(texts.length * this.contextLength);

		for (let i = 0; i < allTokens.length; i++) {
			let tokens = allTokens[i];
			if (tokens.length > this.contextLength) {
				tokens = tokens.slice(0, this.contextLength); // Truncate
				tokens[tokens.length - 1] = BigInt(this.eotTokenId); // Set the last token to EOT
			}
		...
		return new ort.Tensor('int64', flatResult, [texts.length, this.contextLength]);
	}
	...
	private createByteEncoderDecoder(): {
		...
		const bytes: number[] = [
			...
			...Array.from({ length: this.ord('~') - this.ord('!') + 1 }, (_, i) => this.ord('!') + i),

			// Latin-1 characters (161-172)
			...Array.from({ length: this.ord('¬') - this.ord('¡') + 1 }, (_, i) => this.ord('¡') + i),
			// Latin-1 characters (174-255)
			...Array.from({ length: this.ord('ÿ') - this.ord('®') + 1 }, (_, i) => this.ord('®') + i)
		];
	...
	private readMerges(bpePath: string): string[] {
		...
		const content = zlib.gunzipSync(arrayBuffer).toString('utf-8');
	...
	private createEncoderDecoder(vocab: Vocab): {
		...
		const encoder = new Map<string, number>(vocab.map((token, index) => [token, index]));
		const decoder = new Map<number, string>(vocab.map((token, index) => [index, token]));
	...
	private canonicalizeText(
		...
		if (keepPunctuationExactString) {
			text = text
				...
				.map((part) =>
					part.replace(new RegExp(`[${Object.keys(transPunctuation).join('')}]`, 'g'), '')
				)
				.join(keepPunctuationExactString);
		} else {
			text = text.replace(new RegExp(`[${Object.keys(transPunctuation).join('')}]`, 'g'), '');
		}
	...
	getReductionMaskFn(reductionType: ReductionFunctions): ReductionFunction {
	...
	simpleMaskTokenize: ReductionFunction = (texts: string | string[]): ort.Tensor => {
		...
		const resultData = new Int32Array(textArray.length * this.contextLength).fill(0);
		...
		for (let i = 0; i < allTokens.length; i++) {
			...
			if (numTokens > this.contextLength - 2) {
				...
				const startIndex = Math.floor(Math.random() * (numTokens - numKeep + 1)); // inclusive range
		...
		return new ort.Tensor('int32', resultData, [textArray.length, this.contextLength]);
	};
	 ...
	 */
	private randomMaskTokenize = (texts: string | string[], shuffle = false): ort.Tensor => {
	 ...
	 */
	private syntaxMaskTokenize: ReductionFunction = (texts: string | string[]): ort.Tensor => {
		// Ensure texts is an array
		const textArray = typeof texts === 'string' ? [texts] : texts;
		...
		const newTexts = textArray.map((text) => {
			...
			const sampledIndices = sortedIndices.slice(0, this.contextLength - 2).sort((a, b) => a - b);

...
let visionModel: ort.InferenceSession | null = null;
...
async function generateTextEmbeddings(inputs: ort.Tensor): Promise<BatchInferenceResponse> {
	const textModel = await loadTextModel();

	const { text_embeddings } = await textModel.run({ text: inputs });
